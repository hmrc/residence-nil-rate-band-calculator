...
class Calculator @Inject() (env: Environment) {
  ...
  def personsFormerAllowance(
    ...
    val availableValueBeingTransferred =
      if (datePropertyWasChanged.isBefore(legislativeStartDate)) 0 else valueAvailableWhenPropertyChanged
  ...
  def adjustedValueBeingTransferred(totalAllowance: Int, amountToTaper: Int, valueBeingTransferred: Int): Int = {
    ...
    math
      .max(valueBeingTransferred - (amountToTaper.toDouble * (valueBeingTransferred.toDouble / totalAllowance)), 0.0)
      .toInt
  }

  def lostRelievableAmount(
  ...
  def downsizingAllowance(
    ...
    downsizingDetails match {
      ...
      case Some(details) =>
        ...
        val formerAllowance = personsFormerAllowance(
          details.datePropertyWasChanged,
          rnrnOnPropertyChange,
          details.valueAvailableWhenPropertyChanged,
          adjustedBroughtForward
        )
        val adjustedAllowance = taperedAllowance(totalAllowance, amountToTaper)
        val lostAmount =
          lostRelievableAmount(details.valueOfChangedProperty, formerAllowance, propertyValue, adjustedAllowance)
  ...
  def apply(input: CalculationInput): Try[CalculationResult] =
    ...
    for {
      ...
      val downsizingAddition = downsizingAllowance(
        input.downsizingDetails,
        rnrbOnPropertyChange,
        defaultAllowance,
        amountToTaper,
        input.valueBeingTransferred,
        input.propertyValue
      )

      val residenceNilRateAmount =
        math.min(input.propertyValuePassedToDirectDescendants + downsizingAddition, adjustedAllowance)
