...
case class CalculationInput(
  ...
  require(
    ...
    """{"percentagePassedToDirectDescendants" : "error.expected.number.non_negative"}"""
  )

  require(
    percentagePassedToDirectDescendants <= 100,
    """{"percentagePassedToDirectDescendants" : "error.expected.number.100_at_most"}"""
  )
...
object CalculationInput {
  ...
  private def extractErrors(throwable: Throwable): Seq[(String, String)] =
    Json
      .parse(throwable.getMessage.stripPrefix("requirement failed: "))
      .as[JsObject]
      .fields
      .map(error => (error._1, error._2.as[String]))
      .toSeq

  def apply(json: JsValue): Either[Seq[(String, String)], CalculationInput] =
    Try(Json.fromJson[CalculationInput](json)) match {
      case Success(calculationInput) =>
...
case class DownsizingDetails(
  ...
  require(
    ...
    """{"valueAvailableWhenPropertyChanged" : "error.expected.number.non_negative"}"""
  )
